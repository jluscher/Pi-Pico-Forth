/******************************************************************************/
/* DJW                                                                        */
/* Changed ambiguous Types to intNN_t Types                                   */
/******************************************************************************/
...
# define	pushR rack[++R]
//
int32_t rack[STACKSIZE] = { 0 }; 
int32_t stack[STACKSIZE] = { 0 }; 
int64_t d, n, m; 
int8_t R = 0; 
int8_t S = 0; 
int32_t top = 0;
...
void ummod(void)
{
	...
	n = (long long int)((unsigned long)stack[S - 1]); 
	n += m << 32;
	pop;
	top = (unsigned long)(n / d);
	stack[S] = (unsigned long)(n % d);   
}
//
void msmod(void)
{
	...
	n = (signed long long int)((signed long)stack[S - 1]); 
	n += m << 32;
	pop;
	top = (signed long)(n / d);
	stack[S] = (signed long)(n % d); 
}
//
void slmod(void)
{
...
void umsta(void)
{
	...
	m = (unsigned long long int)stack[S]; 
	m *= d;
	top = (unsigned long)(m >> 32);
	stack[S] = (unsigned long)m; 
}
//
void star(void)
{
...
void ssmod(void)
{
	d = (signed long long int)top;
	m = (signed long long int)stack[S];
	n = (signed long long int)stack[S - 1]; 
	n *= m;
	pop;
	top = (signed long)(n / d);
	stack[S] = (signed long)(n % d); 
...
int main()
{
	//cData = (unsigned char*)data;  already done at top of file
	P = 512;
	R = 0;
