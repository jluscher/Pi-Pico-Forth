...
void putkey(char c){ serial_putchar(c); } // wrapper
...
byte readWord()
{
    ...
    return len; // does not return if just <cr> or <lf>
}
...
BUILTIN(38, "QUIT", quit, 0)
{
    ...
    for (exitReq = 0; exitReq == 0;)
    {
        ...
        if (address) // I think this means word has been found, djw
        {
        ...
        else
        {
            ...
            if (notRead) // not a recognized word or a number,... djw
            {
                tell("Unknown word: ");
            ...
            {
                if (*state) // 0 == IMMEDIATE, 1 == COMPILE
                {
                    *((dcell*)tmp) = number;
                    push(lit_id);
                    comma();
